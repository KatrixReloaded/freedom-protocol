...
import { bodyAddress, bodyChainId, bodySeries, bodyString, encode, isNative, txResponse } from "./common.js";
export function buildApproveCollateral(ctx, body) {
...
export function buildPublicMerge(ctx, body) {
    const { series } = bodySeries(ctx, body);
    const amount = BigInt(bodyString(body, "amount"));
    return txResponse({
        chainId: series.chainId,
...
export function buildPublicRedeem(ctx, body) {
    const { series } = bodySeries(ctx, body);
    return txResponse({
        chainId: series.chainId,
        to: series.factoryAddress,
...
export function buildFundBridgeReserve(ctx, body) {
    const { series } = bodySeries(ctx, body);
    const amount = BigInt(bodyString(body, "amount"));
    const deployment = ctx.registry.publicFactory(series.chainId, series.factoryAddress);
    if (!deployment)
