...
import { loadDeploymentRegistry, type DeploymentRegistry } from "./config/deployments.js";
import { loadEnv } from "./env.js";
import { openDb, seedDeployments, type Db } from "./db/client.js";
...
import {
  ...
  buildFundBridgeReserve,
  buildPublicMerge,
  buildPublicRedeem,
  buildPublicSplit
} from "./tx/public.js";
import { openApiSpec } from "./openapi.js";
import { createWorker, type IndexerWorker } from "./indexer/worker.js";
...
type Handler = (ctx: AppContext, req: RequestData) => Promise<unknown> | unknown;
...
function route(pattern: string, path: string): Record<string, string> | undefined {
  const pp = pattern.split("/").filter(Boolean);
  const ap = path.split("/").filter(Boolean);
  if (pp.length !== ap.length) return undefined;
  const params: Record<string, string> = {};
  for (let i = 0; i < pp.length; i++) {
    if (pp[i].startsWith(":")) params[pp[i].slice(1)] = decodeURIComponent(ap[i]);
    else if (pp[i] !== ap[i]) return undefined;
  }
  return params;
}

async function readOnChain(ctx: AppContext, chainId: number, fn: (client: NonNullable<ReturnType<ChainClients["get"]>>) => Promise<unknown>) {
  const client = ctx.chains.get(chainId);
  if (!client) return { available: false, reason: "rpc_not_configured" };
  return { available: true, value: await fn(client) };
}

function txCtx(ctx: AppContext) {
...
const routes: Array<{ method: string; pattern: string; handler: Handler }> = [
  ...
  },
  {
    ...
    handler: (ctx, req) => {
      ...
      if (!series) throw Object.assign(new Error("Unknown series"), { statusCode: 404 });
      return series;
    }
  },
  {
  ...
  { method: "POST", pattern: "/tx/public/fund-bridge-reserve", handler: (ctx, req) => buildFundBridgeReserve(txCtx(ctx), req.body) },
  {
    ...
    handler: (ctx, req) => {
      ...
      if (!factory) throw Object.assign(new Error("Unknown public factory"), { statusCode: 404 });
  ...
  },
  {
    ...
    handler: async (ctx, req) => {
      ...
      if (!user) throw Object.assign(new Error("Missing user"), { statusCode: 400 });
      return readOnChain(ctx, Number(req.params.chainId), (client) =>
        client.readContract({ address: getAddress(req.params.token), abi: ctx.abis.get("IERC20"), functionName: "balanceOf", args: [getAddress(user)] })
      );
    }
  },
  {
    ...
    handler: async (ctx, req) => {
      ...
      if (!owner || !spender) throw Object.assign(new Error("Missing owner or spender"), { statusCode: 400 });
      return readOnChain(ctx, Number(req.params.chainId), (client) =>
        client.readContract({
  ...
  { method: "POST", pattern: "/tx/confidential/redeem", handler: (ctx, req) => buildConfidentialRedeem(txCtx(ctx), req.body) },
  {
    ...
    handler: async (ctx, req) => {
      ...
      const user = req.query.get("user");
      if (!chainId || !token || !user) throw Object.assign(new Error("Missing chainId, token, or user"), { statusCode: 400 });
      return readOnChain(ctx, chainId, (client) =>
        client.readContract({ address: getAddress(token), abi: ctx.abis.get("OptionToken"), functionName: "balanceOf", args: [getAddress(user)] })
      );
    }
  },
  {
    ...
    handler: async (ctx, req) => {
      ...
      const owner = req.query.get("owner");
      const spender = req.query.get("spender");
      if (!chainId || !token || !owner || !spender) throw Object.assign(new Error("Missing chainId, token, owner, or spender"), { statusCode: 400 });
      return readOnChain(ctx, chainId, (client) =>
        client.readContract({
  ...
  },
  {
    ...
    handler: (ctx, req) => {
      ...
      if (!row) throw Object.assign(new Error("Unknown bridge request"), { statusCode: 404 });
      return row;
    }
  },
  {
    ...
    handler: (ctx, req) => {
      ...
      const row = ctx.db.prepare("SELECT * FROM bridge_requests WHERE request_key=?").get(req.params.requestKey);
      if (!row) throw Object.assign(new Error("Unknown bridge request"), { statusCode: 404 });
      if (req.body?.bridgeMintable != null) markBridgeRetryability(ctx.db, req.params.requestKey, BigInt(req.body.bridgeMintable));
      return row;
    }
  },
  ...
  },
  {
    ...
    handler: (ctx, req) => {
      ...
      if (!row) throw Object.assign(new Error("Unknown listing"), { statusCode: 404 });
      return row;
    }
  },
  ...
  },
  {
    ...
    handler: (ctx, req) => {
      ...
      if (!row) throw Object.assign(new Error("Unknown pool"), { statusCode: 404 });
      return row;
    }
  },
...
export async function dispatch(ctx: AppContext, method: string, pathWithQuery: string, body?: any): Promise<{ status: number; body: unknown }> {
  try {
...
let worker: IndexerWorker | undefined;
