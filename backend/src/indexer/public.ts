import { decodeEventLog, getAddress, type Log, type Abi, zeroAddress } from "viem";
...
export function processPublicLog(db: Db, registry: DeploymentRegistry, abis: AbiBundle, chainId: number, log: Log): boolean {
  ...
  if (!abi) return false;
  const decoded = decodeEventLog({ abi: abi as Abi, data: log.data, topics: log.topics, strict: false });
  ...
  if (factory && decoded.eventName === "Settled") {
    db.prepare(
      "UPDATE series SET settled=1,stable_payout=?,up_payout=?,updated_at=CURRENT_TIMESTAMP WHERE chain_id=? AND factory_address=? AND series_id=?"
    ).run(args.stablePayout.toString(), args.upPayout.toString(), chainId, factory.factory, args.seriesId);
  }
  if (factory && decoded.eventName === "BridgeReserveFunded") {
    const row = db
      ...
      .get(chainId, factory.factory, args.seriesId) as { series_key: string } | undefined;
    if (row) upsertReserve(db, chainId, row.series_key, BigInt(args.amount), BigInt(args.amount), log.blockNumber ?? undefined);
  }
  if (factory && decoded.eventName === "BridgeMinted") {
    const row = db
      .prepare("SELECT series_key FROM series WHERE chain_id=? AND factory_address=? AND series_id=?")
      .get(chainId, factory.factory, args.seriesId) as { series_key: string } | undefined;
    if (row) upsertReserve(db, chainId, row.series_key, 0n, -BigInt(args.amount), log.blockNumber ?? undefined);
  }
  if (vault && decoded.eventName === "ReserveDeposited") {
    const row = db
      .prepare("SELECT series_key FROM series WHERE chain_id=? AND factory_address=? AND series_id=?")
      .get(chainId, vault.factory, args.seriesId) as { series_key: string } | undefined;
    if (row) upsertReserve(db, chainId, row.series_key, BigInt(args.amount), 0n, log.blockNumber ?? undefined);
  }
