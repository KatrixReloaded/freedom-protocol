...
function addressesFor(ctx: AppContext, chainId: number): Address[] {
  const chain = ctx.registry.chain(chainId);
  if (!chain) return [];
  const addresses: Address[] = [];
  for (const factory of chain.publicFactories) addresses.push(factory.factory, factory.vault);
  for (const factory of chain.confidentialFactories) addresses.push(factory.factory, factory.vault);
  if (chain.bridge) addresses.push(chain.bridge);
  if (chain.matchingEngine) addresses.push(chain.matchingEngine);
  const pools = ctx.db.prepare("SELECT pool_address FROM pools WHERE chain_id=?").all(chainId) as { pool_address: Address }[];
  for (const pool of pools) addresses.push(pool.pool_address);
  return [...new Set(addresses)];
}

export async function indexRange(ctx: AppContext, chainId: number, fromBlock: bigint, toBlock: bigint): Promise<number> {
  ...
  for (const log of logs as Log[]) {
    try {
      if (
        processPublicLog(ctx.db, ctx.registry, ctx.abis, chainId, log) ||
...
export function createWorker(ctx: AppContext): IndexerWorker {
