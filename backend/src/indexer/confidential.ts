...
export function processConfidentialLog(db: Db, registry: DeploymentRegistry, abis: AbiBundle, chainId: number, log: Log): boolean {
  ...
  const decoded = decodeEventLog({ abi: abis.get("OptionFactory") as Abi, data: log.data, topics: log.topics, strict: false });
  ...
  if (decoded.eventName === "Settled") {
    db.prepare("UPDATE series SET settled=1,updated_at=CURRENT_TIMESTAMP WHERE chain_id=? AND factory_address=? AND series_id=?").run(
      chainId,
      factory.factory,
      args.seriesId
    );
  ...
  if (decoded.eventName === "PoolCreated") {
    const key = seriesKey(chainId, factory.factory, args.strike, args.maturity);
    db.prepare(
      "INSERT INTO pools (chain_id,pool_address,factory_address,series_key,strike,maturity,is_stable,created_block) VALUES (?, ?, ?, ?, ?, ?, ?, ?) " +
        "ON CONFLICT(pool_address) DO UPDATE SET series_key=excluded.series_key"
    ).run(chainId, getAddress(args.pool), factory.factory, key, args.strike.toString(), args.maturity.toString(), args.isStable ? 1 : 0, Number(log.blockNumber ?? 0n));
  }
  return true;
}
