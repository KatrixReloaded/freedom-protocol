...
export function buildPoolDeposit(ctx: BuilderContext, body: any) {
...
export function buildPoolFill(ctx: BuilderContext, body: any) {
  const chainId = Number(body.chainId);
  const pool = bodyAddress(body, "pool");
...
export function buildPoolWithdraw(ctx: BuilderContext, body: any) {
  const chainId = Number(body.chainId);
