...
import { bodySeries, bodyString, encode, txResponse, type BuilderContext } from "./common.js";

export function buildConfidentialSplit(ctx: BuilderContext, body: any): TxResponse {
...
export function buildConfidentialMerge(ctx: BuilderContext, body: any): TxResponse {
  const { series } = bodySeries(ctx, body);
...
export function buildConfidentialRedeem(ctx: BuilderContext, body: any): TxResponse {
  const { series } = bodySeries(ctx, body);
  return txResponse({
    chainId: series.chainId,
    to: series.factoryAddress,
