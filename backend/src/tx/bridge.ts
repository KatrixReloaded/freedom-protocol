...
import { bodyBool, bodyChainId, bodyString, encode, txResponse, type BuilderContext } from "./common.js";

export function buildBridgeUnshield(ctx: BuilderContext, body: any): TxResponse {
  const chainId = bodyChainId(body);
  const chain = ctx.registry.chain(chainId);
  if (!chain?.bridge) throw Object.assign(new Error("Bridge not configured"), { statusCode: 404 });
  ...
  if (amount > 18_446_744_073_709_551_615n) throw Object.assign(new Error("Amount exceeds uint64.max"), { statusCode: 400 });
  return txResponse({
    chainId,
    to: chain.bridge,
...
export function buildBridgeFinalize(ctx: BuilderContext, body: any): TxResponse {
  const chainId = bodyChainId(body);
  const chain = ctx.registry.chain(chainId);
  if (!chain?.bridge) throw Object.assign(new Error("Bridge not configured"), { statusCode: 404 });
  ...
  const actualBurned = decodeActualBurned({ abiEncodedCleartexts: abiEncodedCleartexts as `0x${string}`, decryptionProof: decryptionProof as `0x${string}` });
  return txResponse({
    chainId,
    to: chain.bridge,
