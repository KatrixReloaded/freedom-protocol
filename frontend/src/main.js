...
function ensureMotionBackground() {
  ...
  const canvas = h("canvas", { class: "motion-canvas", "aria-hidden": "true" });
  document.body.insertBefore(canvas, document.querySelector("#app"));

  const ctx = canvas.getContext("2d");
  if (!ctx) return;
  const reduced = window.matchMedia("(prefers-reduced-motion: reduce)");
  const pointer = { x: -9999, y: -9999, vx: 0, vy: 0, active: 0 };
  const particles = [];
  let width = 0;
  let height = 0;
  let dpr = 1;
  ...
  function seedParticles() {
    particles.length = 0;
    const count = Math.min(4200, Math.max(1200, Math.floor((width * height) / 360)));
  ...
  function draw(time) {
    ...
    for (const particle of particles) {
      ...
      const radius = 210;
      if (pointer.active > 0.01 && distSq < radius * radius) {
        const dist = Math.sqrt(distSq) || 1;
        const force = (1 - dist / radius) * pointer.active;
      ...
      const alpha = 0.035 + particle.depth * 0.105 + nearPointer * 0.34 * pointer.active;
      const size = particle.size + nearPointer * 2.2 * pointer.active;
      ctx.fillStyle = `rgba(245,200,75,${alpha})`;
